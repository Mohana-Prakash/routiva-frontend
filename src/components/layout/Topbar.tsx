...
function currentTitle(pathname: string | null): string {
  return NAV_ITEMS.find((item) => pathname?.startsWith(item.href))?.label ?? env.appName;
}
...
export function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b bg-background/95 px-4 backdrop-blur supports-backdrop-filter:bg-background/80 md:px-6">
      ...
      <h1 className="hidden text-base font-semibold md:block">{currentTitle(pathname)}</h1>
      <UserMenu />
    </header>
  );
}
