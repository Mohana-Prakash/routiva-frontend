...
export function useTrackingAvailability(item: ScheduleDayItem | null, date: string, timezone: string) {
  const now = useNow();
  if (!item) {
    return { isTimeless: true, timeHasArrived: false, timeHasEnded: false, canStart: false, canComplete: false, canSkip: false };
  }
  ...
  return { isTimeless, timeHasArrived, timeHasEnded, canStart, canComplete, canSkip };
}
