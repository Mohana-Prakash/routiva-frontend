...
export function ActivityPerformanceList({
  items,
}: {
          ...
          <Card>
            <CardContent className="space-y-2 p-3">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground mt-1">
                <span>
                  Planned: {formatDurationMinutes(item.plannedMinutes)}
                </span>
                <span>Actual: {formatDurationMinutes(item.actualMinutes)}</span>
                <span>Achievement: {Math.round(item.achievementRate)}%</span>
              </div>
              <Progress
                value={Math.min(100, item.achievementRate)}
                className="h-1.5"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Consistency: {item.consistencyCompletedSessions} /{" "}
                {item.consistencyTotalSessions} sessions (
                {Math.round(item.consistencyRate)}%)
              </p>
            </CardContent>
          </Card>
        </li>
